...
CREATE TRIGGER MaxItemsPerBagTrig 
    ...
    FOR EACH ROW 
    BEGIN 
        SET @num_items = (SELECT Account.NumItemsPerUser FROM (SELECT UserId, COUNT(*) AS NumItemsPerUser FROM BagItems GROUP BY UserId) Account WHERE Account.UserId = new.UserId); 
